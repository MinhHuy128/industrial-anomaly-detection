...
def compute_anomaly_map(en_list, de_list, crop_size: int = 392, out_size: int = 448) -> np.ndarray:
    ...
    for en, de in zip(en_list, de_list):
        a_map = 1.0 - F.cosine_similarity(en, de, dim=1, eps=1e-8)
    ...
    crop_amap   = anomaly_map.squeeze().cpu().numpy()
...
def evaluate(args):
    ...
    model_name    = "ViTill-GCT V2 (Proposed)" if use_gct else "Comparative Baseline"
    ...
    if ckpt_path.exists():
        ...
        if verbose:
            print(f"[CKPT] Loaded checkpoint: {ckpt_path.relative_to(ROOT)}")
    else:
        raise FileNotFoundError(
            f"Checkpoint not found at '{ckpt_path}'. "
            f"Pre-trained weights are required for evaluation. "
            f"Please train the model first: python src/train.py --category {category} {'--use_gct' if use_gct else ''} "
            f"or provide a valid checkpoint. Official benchmark results are available in results/benchmark_official.json."
        )
    ...
    if verbose:
        print(f"[METRICS] Logical AUROC   : {log_auroc:.2f}%  (F1: {log_f1:.2f}%)")
        print(f"[METRICS] Structural AUROC: {struct_auroc:.2f}%  (F1: {struct_f1:.2f}%)")
        print(f"[METRICS] Mean AUROC      : {mean_auroc:.2f}%  (Mean F1: {mean_f1:.2f}%)")
        print(f"[METRICS] sPRO            : {spro:.2f}%")
        print(f"[LATENCY] {latency_ms:.2f} ms/image ({fps:.1f} FPS)")
    else:
        print(f"[{category}] Logical: {log_auroc:.2f}% | Structural: {struct_auroc:.2f}% | Mean: {mean_auroc:.2f}% | sPRO: {spro:.2f}%")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gamma",     type=float, default=None, help="GCT weight gamma (default: 1.0)")
    parser.add_argument("--save_maps", action="store_true", help="Save anomaly maps to disk")
