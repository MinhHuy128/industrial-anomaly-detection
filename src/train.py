...
def train(args):
    ...
    log_path = save_path / f"{category}_train.log"
    ...
    log_message(f"[RUN] {timestamp} | model={model_name} | category={category}")
    log_message(f"[TRAIN] Starting iteration-based loop for {TOTAL_ITERS} iterations...")
    ...
    for it in range(TOTAL_ITERS):
        ...
        if (it + 1) % 100 == 0:
            ...
            msg = (f"  [Iter {it+1:05d}/{TOTAL_ITERS}] "
                   f"Loss: {avg_loss:.6f}  "
