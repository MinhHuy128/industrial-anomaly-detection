...
def binary_refinement(init_queries: Sequence,
    ...
    # Remove any potential duplicate queries to maintain strict monotonicity
    clean_queries, clean_values = [], []
    for q, v in zip(all_queries, all_values):
        if not clean_queries or (ascending and q > clean_queries[-1]) or (not ascending and q < clean_queries[-1]):
            clean_queries.append(q)
            clean_values.append(v)

    # Start a new refinement step.
    return binary_refinement(init_queries=clean_queries,
                             init_values=clean_values,
